...
> 
> Ein Visual Web Developer-Projekt mit c#-Quellcode ist für dieses Thema verfügbar. [Laden Sie die c#-Version herunter](https://code.msdn.microsoft.com/Introduction-to-MVC-3-10d1b098). Wenn Sie Visual Basic bevorzugen, wechseln Sie zur [Visual Basic-Version](../vb/intro-to-aspnet-mvc-3.md) dieses Tutorials.
...
[![HTML. Action Link](examining-the-edit-methods-and-edit-view/_static/image4.png)](examining-the-edit-methods-and-edit-view/_static/image3.png)
...
Wenn Sie auf die Schaltfläche **Hinzufügen** klicken, wird die Ansichts Vorlage *views\movies\searchindex.cshtml* erstellt. Da Sie **Liste** in der Liste **Gerüstbau Vorlage** ausgewählt haben, generiert Visual Web Developer automatisch einige Standard Inhalte in der Ansicht. Der Gerüstbau hat ein HTML-Formular erstellt. Die `Movie` Klasse wurde untersucht und Code erstellt, um `<label>` Elemente für jede Eigenschaft der Klasse zu erzeugen. In der folgenden Liste wird die erstellte Create View angezeigt:
...
Die Lösung besteht in der Verwendung einer Überladung von `BeginForm` , die angibt, dass die Post-Anforderung die Suchinformationen der URL hinzufügen soll und dass an die HttpGet-Version der Methode weitergeleitet werden soll `SearchIndex` . Ersetzen Sie die vorhandene Parameter lose- `BeginForm` Methode durch Folgendes:
