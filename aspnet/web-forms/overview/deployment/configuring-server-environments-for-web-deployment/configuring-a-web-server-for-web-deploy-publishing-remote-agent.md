...
> In diesem Thema wird beschrieben, wie Sie einen Internetinformationsdienste (IIS)-Webserver konfigurieren, um die Webveröffentlichung und-Bereitstellung mit dem Remote-Agent-Dienst IIS Web Deployment Tool (Web deploy) zu unterstützen.
> 
> Wenn Sie mit Web deploy 2,0 oder höher arbeiten, gibt es drei Hauptansätze, die Sie verwenden können, um Ihre Anwendungen oder Websites auf einen Webserver zu übernehmen. Sie können:
> 
...
Ja, wenn der Benutzer, der den Inhalt bereitstellt, die Anmelde Informationen eines Administrators auf dem Zielserver bereitstellen kann. Diese Vorgehensweise ist häufig in folgenden Szenarios wünschenswert:

- Entwicklungs-oder Testumgebungen, in denen der Entwickler über Vollzugriff auf den Zielweb Server und den Datenbankserver verfügt.
- Kleinere Unternehmen, in denen ein einzelner Benutzer oder eine kleine Gruppe von Benutzern die Kontrolle über den gesamten Anwendungslebenszyklus hat.

In vielen größeren Unternehmen, insbesondere bei Staging-oder Produktionsumgebungen, ist es oft nicht realistisch, Benutzern Administratorrechte für Webserver zu übermitteln. Im Fall von gehosteten Webservern ist dies besonders unwahrscheinlich. Wenn Sie beabsichtigen, die Bereitstellung von einem Buildserver zu automatisieren, sollten Sie außerdem möglicherweise keine Administrator Anmelde Informationen für den Bereitstellungs Prozess verwenden. In diesen Szenarien kann das Konfigurieren von Webservern für die Unterstützung der Bereitstellung mithilfe des [Web deploy Handlers](configuring-a-web-server-for-web-deploy-publishing-web-deploy-handler.md) zu einer zufriedeneren Wahl werden.

## <a name="task-overview"></a>Aufgaben Übersicht

In diesem Thema wird beschrieben, wie Sie einen Internetinformationsdienste (IIS) 7,5-Webserver so konfigurieren, dass Webpakete von einem Remote Computer mithilfe des Web deploy Remote-Agent-Ansatzes akzeptiert und bereitgestellt werden. Folgendes ist erforderlich:
...
- **Webbereitstellungs Tool 2,1 oder**höher. Hierdurch werden Web deploy (und die zugrunde liegende ausführbare Datei MSDeploy.exe) auf dem Server installiert. Im Rahmen dieses Prozesses wird der Web Deployment Agent-Dienst installiert und gestartet. Mit diesem Dienst können Sie Webpakete von einem Remote Computer aus bereitstellen.
- **ASP.NET MVC 3**. Dadurch werden die Assemblys installiert, die Sie zum Ausführen von MVC 3-Anwendungen benötigen

> [!NOTE]
> In dieser exemplarischen Vorgehensweise wird die Verwendung des Webplattform-Installers zum Installieren und Konfigurieren der erforderlichen Komponenten beschrieben. Obwohl Sie den Webplattform-Installer nicht verwenden müssen, wird der Installationsvorgang vereinfacht, da Abhängigkeiten automatisch erkannt werden und sichergestellt wird, dass Sie immer die neuesten Produktversionen erhalten. Weitere Informationen finden Sie unter [Microsoft-Webplattform-Installer 3,0](https://go.microsoft.com/?linkid=9805118).
...
3. Erweitern Sie im IIS-Manager im Bereich **Verbindungen** den Server Knoten (z. b. **TESTWEB1**).
...
12. Geben Sie im Feld **Hostname** den Namen des Webservers ein (z. b. **TESTWEB1**), und klicken Sie dann auf **OK**.

    ![](configuring-a-web-server-for-web-deploy-publishing-remote-agent/_static/image6.png)

    > [!NOTE]
    > Mit der ersten Site Bindung können Sie lokal auf die Website zugreifen, indem Sie die IP-Adresse und den Port oder verwenden `http://localhost:85` . Die zweite Site Bindung ermöglicht den Zugriff auf die Website von anderen Computern in der Domäne mithilfe des Computer namens (z. b http://testweb1:85) .).
...
- Weisen Sie der Anwendungs Pool Identität direkt Berechtigungen zu, und verwenden Sie dabei das Format **IIS AppPool \[ Application Pool Name]** (z. b. **IIS apppool\demosite**).
...
Als letzte Aufgabe sollten Sie sicherstellen, dass der Web Deployment Agent-Dienst ausgeführt wird, bevor Sie versuchen, Webanwendungen auf dem Server bereitzustellen. Wenn Sie ein Paket von einem Remote Computer aus bereitstellen, ist der Web Deployment Agent-Dienst für das Extrahieren und Installieren der Inhalte des Pakets verantwortlich. Der Dienst wird standardmäßig gestartet, wenn Sie das Webbereitstellungs Tool installieren und unter der Netzwerkdienst Identität ausführen.

Mithilfe verschiedener Befehlszeilen Programme oder Windows PowerShell-Cmdlets können Sie überprüfen, ob ein Dienst auf unterschiedliche Weise ausgeführt wird. In diesem Verfahren wird ein einfacher Benutzeroberflächen basierter Ansatz beschrieben.
...
Standardmäßig lauscht der Remote-Agent-Dienst unter dieser URL an TCP-Port 80:
